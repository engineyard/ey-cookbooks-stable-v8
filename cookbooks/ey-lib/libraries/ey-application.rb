  ...
  module EY
    class Application
      ...
      def initialize(name, app, node, root = "/data")
        ...
        @database_username = ey_app["database_username"] || node["dna"]["engineyard"]["environment"]["ssh_username"]
        @database_password = ey_app["database_password"] || node["dna"]["engineyard"]["environment"]["ssh_password"]
        @root = root
      ...
      def default_paths
        ["shared", "shared/bin", "shared/config", "shared/pids", "shared/system", "releases" ].map do |dir|
          File.join(@path, dir)
        end
      end
      ...
      def metadata(key = nil, default = nil)
        unless @component_metadata
          @component_metadata = ey_app["components"].detect { |c| c["key"] == "app_metadata" }.dup.reject { |k| k == "key" }
        end
      ...
      def app_server_recipes
        ["nginx", "trinidad", "thin", "puma", "passenger", "passenger3", "passenger4",
          ...
          "unicorn", "node::standard", "node::tcp", "passenger6", "puma_legacy"]
      end

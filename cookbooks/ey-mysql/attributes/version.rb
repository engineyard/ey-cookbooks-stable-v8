...
default["latest_version_80"] = `apt-cache madison percona-server-common | grep -E '^ *percona-server-common \\| *8\\.0' | sort -V | awk '{print $3'} | tail -1`.split("-")[0]
default["latest_version_84"] = `apt-cache madison percona-server-common | grep -E '^ *percona-server-common \\| *8\\.4' | sort -V | awk '{print $3'} | tail -1`.split("-")[0]
...
when "mysql8_4", "aurora8_4"
  major_version = "8.4"
  default["mysql"]["latest_version"] = node["latest_version_84"]
end

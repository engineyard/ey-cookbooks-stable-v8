...
# PG12+ / PG16: promote a standby with pg_promote(), not a trigger file.
# The trigger-file promotion GUC was removed in PostgreSQL 16 and would raise
# an unknown-parameter error if written to postgresql.conf.
execute "promote standby" do
  command %(psql -U postgres -c "SELECT pg_promote();")
  only_if { node["failover"] }
end
